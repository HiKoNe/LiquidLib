    ...
    internal static class ILHooks
    {
        ...
        static void Liquid_Update(ILContext il)
        {
            ...
            while (c.TryGotoNext(i => i.MatchLdloc(12)))
            {
                ...
                c.Remove();
                c.Emit(OpCodes.Ldarg_0);
                c.EmitDelegate<Func<Liquid, bool>>((liquid) =>
                {
                    if (!WorldGen.SolidTile(liquid.x, liquid.y, false))
                    {
                        ...
                        {
                            if (LiquidLoader.liquidCollisions.Contains(tile.LiquidType, t.LiquidType))
                                return LiquidLoader.liquidCollisions[tile.LiquidType, t.LiquidType];
                            return new LiquidCollision(0, 0) { TileType = TileID.Dirt, Sound = SoundID.LiquidsWaterLava };
                        }
